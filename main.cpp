/*
 * This file is part of the CryptoLib256 library.
 * Developed (D) 2021/2022 by Wayne Michael Thornton aka WMT.
 * 
 * Distributed under the Public Benefit Zero Copyright License (v.1.0)
 * 
 * You should have received a copy of the Public Benefit Zero Copyright License
 * along with this program. If not, see <https://github.com/wmthornton/PBZC.git>.
*/

// DEMO FILE ** DEMO FILE ** DEMO FILE ** DEMO FILE ** DEMO FILE ** DEMO FILE ** //

#include "CryptoLib256/CryptoLib256.h" // A precompiled header file is included which speeds up compilation. This file is called CryptoLib256.h.gch

int main(){
    
    SHA256_ENCRYPT::u8 output[32]; // Generate an empty 32 byte output array
    SHA256_ENCRYPT::u8 input[32] = {
        0x66,0xd5,0x4e,0x28,0x5f,0xff,0x6b,0x59,0xac,0x3b,0x34,0x14,0xb5,0x3c,0xb2,0xc6,
        0xa4,0x85,0x1e,0x0d,0x86,0xc7,0x4f,0xbb,0x75,0x5e,0xcb,0xc3,0x6e,0x48,0x79,0x8f}; // Generate a 32 byte input array. U8 is a typedef for unsigned char.
    SHA256_ENCRYPT::u32 diffusion[32] = {
        0xf26cb481,0x16a5dc92,0x3c5ba924,0x79b65248,0x2fc64b18,0x615acd29,0xc3b59a42,0x976b2584,
        0x6cf281b4,0xa51692dc,0x5b3c24a9,0xb6794852,0xc62f184b,0x5a6129cd,0xb5c3429a,0x6b978425,
        0xb481f26c,0xdc9216a5,0xa9243c5b,0x524879b6,0x4b182fc6,0xcd29615a,0x9a42c3b5,0x2584976b,
        0x81b46cf2,0x92dca516,0x24a95b3c,0x4852b679,0x184bc62f,0x29cd5a61,0x429ab5c3,0x84256b97}; // Generate a 32 byte diffusion array. U32 is a typedef for unsigned int.
    SHA256_ENCRYPT::u8 confusion[512] {
        }; // Generate a 512 byte confusion array. U8 is a typedef for unsigned char.

    int32_t hash_encrypt_output = SHA256_ENCRYPT::generate_hash_value(input, output, confusion, diffusion); // generate_hash_value is a function in the SHA256_ENCRYPT class.

    std::string hash_output = std::to_string(hash_encrypt_output); // Convert the integer result of the generate_hash_value function to a string.
    std::string input_value = "Hello World"; // The input value to be hashed.
    std::string sha256_c = sha256(input_value); // Input value text is run thru the sha256 function.

    std::cout << "Hash output: (Integer value generated by the 'generate_hash_value' function using input, output, confusion and diffusion variables) " << hash_output << std::endl;
    std::cout << "Input result: ('Hello World' run thru sha256 algorithm) " << sha256_c << std::endl;

    std::string sha256_result = sha256(hash_output + sha256_c);

    std::cout << "SHA256 result: (Combination of Hash output and Input result, run thru sha256 algorithm) " << sha256_result << std::endl;

    return 0;
    
}